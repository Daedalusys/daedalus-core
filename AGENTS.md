...
## STRUCTURE
```
Daedalus/                    # 主仓 = daedalus-core 运行时 + copilot + 镜像构建 (四层结构, 决策 23/24 + 25)
...
├── scripts/                 # 仓库级辅助脚本 (镜像外; 与 daedalus-core/files/scripts/ 镜像内构建步骤不同作用域)
│   ├── sync-daedalus.sh         # Syncs tracked daedalus-core/ source tree into base_image vendor tree (targeted stale-delete on systemd leg; --delete-excluded on plugin leg)
│   ├── pack-copilot-plugin.sh   # Copilot Pack→Verify 安装态生成 (源 = daedalus-core/plugin/copilot/)
...
├── daedalus-core/           # ★ 代码逻辑层 + 镜像构建层: Go 模块根 (module github.com/Daedalusys/daedalus-core; 镜像外, 仅 go.mod + go.sum 入库, vendor/ 与 bin/ 均不入库)
│   ├── cmd/                 #   daedalus-{audit,host,plugin-pack,smoke,tx} 二进制入口
│   ├── internal/            #   controller tx 共享包 (决策 25 契约缝锁定; 其余安全包已迁 daedalus-sdk)
│   ├── plugin/              #   官方预置插件层 (源码侧): copilot/ (Deno 源码 + daedalus.plugin.json, 留主仓)
...
├── daedalus-sdk/            # ★ SDK 仓根 (11 安全核心包 + 5 Provider/Slot 占位; module github.com/Daedalusys/daedalus-sdk)
├── daedalus-plugins/        # ★ 插件仓根 (6 Go 能力插件 monorepo: fs/shell/pkg/sysinfo/service/blueprint)
...
| Task | Location | Notes |
|------|----------|-------|
| Go MCP 能力服务器实现 | `daedalus-plugins/{fs,shell,pkg,sysinfo}/cmd/` + `daedalus-sdk/{pathguard,shellpolicy,pkgquery,sysinfo}/` | go-sdk stdio 服务器; 唯一实现 (Python/Deno 双实现已删除) |
| Modify Daedalus Copilot CLI | `daedalus-core/plugin/copilot/` (源码) + `daedalus-core/files/system/opt/daedalus/plugins/daedalus.copilot/` (镜像安装态) | policy, audit, llm, exec, main orchestration; 安装态是构建产物勿手改 |
| 插件格式 / 宿主 | `daedalus-core/cmd/daedalus-{host,plugin-pack}/` + `daedalus-sdk/plugin/` + `daedalus-plugins/<cap>/daedalus.plugin.json` + `daedalus-core/plugin/README.md` | manifest schema + zip 打包 + sha256 + zip-slip 防护; 宿主仅安装/发现/校验 (非父进程) |
| 安全白名单 / 策略 | `daedalus-core/files/system/opt/daedalus/shared/policy.toml` + `daedalus-sdk/policy/` | 单一事实源, Go 运行时读取; ALLOW_COMMANDS env 整体 REPLACE; 损坏 fail-closed 拒启 |
| Copilot 侧冻结副本 | `daedalus-core/plugin/copilot/policy.ts` | 与 `daedalus-sdk/shellpolicy` 同步义务; 契约由 `tests/deno/shellpolicy_contract.test.ts` 钉 |
| Audit hash chain | `daedalus-sdk/audit/` + `daedalus-core/cmd/daedalus-audit/` | genesis `0`*64, syscall.Flock, sha256 链, Python 金样字节级兼容 (`daedalus-sdk/audit/testdata/golden.jsonl`) |
| Add systemd sandbox rule | `daedalus-core/files/system/usr/lib/systemd/system/daedalus-*.service.d/*.conf` | landlock.conf = seccomp/network; credentials.conf = LoadCredential |
| Reorder / add image build step | `daedalus-core/files/scripts/NN-name.sh` | 60-ai-middleware, 65-ai-safety, 70-daedalus-mcp-servers, 75-daedalus-copilot, 76-daedalus-plugin-gen |
...
## CODE MAP
CodeGraph indexes `tests/` and tracked root files (`base_image/` is gitignored). Source paths under `daedalus-core/` (代码逻辑层 + copilot 插件 + 镜像构建层), `daedalus-sdk/` (SDK 包) 与 `daedalus-plugins/` (6 能力插件) are authoritative.

| Symbol / Component | Type | Location | Role |
|--------------------|------|----------|------|
| `daedalus` CLI | wrapper script | `daedalus-core/files/system/usr/local/bin/daedalus` | 询问宿主构造启动命令 (不自己声明 Deno 权限), `$HOME` 占位展开后 exec |
...
| `cmd/daedalus-{fs,shell,pkg,sysinfo}` | Go binaries | `daedalus-plugins/{fs,shell,pkg,sysinfo}/cmd/` | 4 个 MCP stdio 能力服务器 (镜像态 = 插件内 `bin/`) |
| `policy.toml` | TOML | `daedalus-core/files/system/opt/daedalus/shared/policy.toml` | 安全策略单一事实源 ([shell]/[fs]/[audit]) |
| `daedalus.plugin.json` | manifest | `daedalus-core/plugin/copilot/` + `daedalus-plugins/{fs,shell,pkg,sysinfo,service,blueprint}/` | 插件声明 (id/type/runtime/executable/entrypoint/permissions/tools) |
| `policy.ts` | TS module | `daedalus-core/plugin/copilot/policy.ts` | Proposal schema validation & frozen shellpolicy copy |
| `audit.ts` | TS module | `daedalus-core/plugin/copilot/audit.ts` | Hash-chained audit logging via daedalus-audit Go CLI |
| `llm.ts` | TS module | `daedalus-core/plugin/copilot/llm.ts` | OpenAI & Anthropic cloud LLM adapters with revision support |
| `exec.ts` | TS module | `daedalus-core/plugin/copilot/exec.ts` | JSON-RPC bridge spawning daedalus-shell Go MCP server with 40s watchdog |
| `main.ts` | TS entry | `daedalus-core/plugin/copilot/main.ts` | Command advisor (命令顾问) flow: L0 y/n sandboxed execution, L1/L2 display-only + manual-execution hint, opt-in interactive loop, edit/revise, REPL |
| `76-daedalus-plugin-gen.sh` | build script | `daedalus-core/files/scripts/76-daedalus-plugin-gen.sh` | 构建期从 manifest+policy.toml 渲染/自校验 systemd ExecStart + tools 交叉核对 |
| `75-daedalus-copilot.sh` | build script | `daedalus-core/files/scripts/75-daedalus-copilot.sh` | Sets directory and executable permissions for Copilot |
...
- **安装态 vs 源码侧**: 镜像安装态 `daedalus-core/files/system/opt/daedalus/plugins/daedalus.service/daedalus.plugin.json` 的 `resources` 字段 (含 `desired_state` 补全与 checksums 注入) 是 `just plugin-pack` 的构建期产物; 源码侧仅 `daedalus-plugins/service/` 声明 `resources` (`{ "kind": "service", "name": "*" }`), 其余 4 个既有能力与 copilot manifest 暂不写。`76-daedalus-plugin-gen.sh` 构建期交叉核对 `resources[].kind` ⊆ `[objectmodel].enabled_kinds`, 漂移即拒构建。
...
- **Policy**: `policy.toml [blueprints]` 节(输出目录白名单 `output_dirs` / post_check 命令白名单 `post_check_commands` / reload 服务白名单 `reload_services` / secret 来源 `secret_sources`);post_check 白名单经 `internal/shellpolicy` 的 `RegisterBlueprintsPostCheckSource` 钩子注入(严格子集,独立于主白名单,与 policy.toml 三点漂移测试钉死)。
...
- **Plugin Host Launch**: Copilot is installed as plugin `daedalus.copilot` (runtime=deno) under `/opt/daedalus/plugins/`; source lives in `daedalus-core/plugin/copilot/`. The `/usr/local/bin/daedalus` wrapper asks `daedalus-host run-plugin daedalus.copilot` to construct (not spawn) the launch command from the manifest, expands `$HOME` placeholders, then execs it:
  ```sh
...
- **代码文件清单**：本项目涉及的主要代码文件包括：
  - **Go (core)**: `daedalus-core/cmd/daedalus-{audit,host,plugin-pack,smoke,tx}/` + `daedalus-core/internal/{controller,tx}/` + `daedalus-sdk/{pathguard,shellpolicy,pkgquery,sysinfo,policy,audit,plugin,objectmodel,i18n,blueprint,version,state,dirs}/` + `daedalus-plugins/{fs,shell,pkg,sysinfo,service,blueprint}/cmd/`
  - **TS / Deno (copilot 源码)**: `daedalus-core/plugin/copilot/{policy,audit,exec,llm,main}.ts` + `daedalus.plugin.json`
  - **TS 测试 (镜像外)**: `tests/deno/{policy,audit,exec,llm,main}.test.ts` + `tests/deno/shellpolicy_contract.test.ts`
  - **Shell scripts**: `daedalus-core/files/scripts/{60-ai-middleware,65-ai-safety,70-daedalus-mcp-servers,75-daedalus-copilot,76-daedalus-plugin-gen}.sh`, `usr/local/bin/daedalus` wrapper, `scripts/sync-daedalus.sh`, `scripts/pack-copilot-plugin.sh`, `scripts/justfile.demo`, `scripts/copilot-prep.sh`
  ...
  - **Policy / manifests**: `daedalus-core/files/system/opt/daedalus/shared/policy.toml`, `daedalus-core/plugin/*/daedalus.plugin.json`
- **四层结构 (决策 23/24 + 25)**: `daedalus-core/` = Go 代码逻辑层 (镜像外) + copilot 插件源码侧 + 镜像构建落位 (`daedalus-core/files/`); `daedalus-sdk/` = SDK 仓 (11 安全核心包 + 5 Provider/Slot 占位); `daedalus-plugins/` = 插件仓 (6 Go 能力插件 monorepo)。镜像内 `opt/daedalus/plugins/` 是**构建产物 (安装态)**, `daedalus-core/plugin/` 与 `daedalus-plugins/` 是**源码侧定义**——两者绝不同步混用。
...
- **Vendor tree = `base_image/`**: Vendored upstream fork (gitignored). Updated from `daedalus-core/files/` (及 `daedalus-core/plugin/` → `base_image/plugin/`, 在 COPY 白名单外) via `scripts/sync-daedalus.sh` before container builds.
...
- **镜像安装态策略**: `daedalus-core/files/system/opt/daedalus/plugins/daedalus.*/bin/` 与 `daedalus-core/files/system/usr/local/bin/daedalus-{audit,host,shell}` 是 `just plugin-pack` 的 Go 编译产物副本,均**不入库**(同 `daedalus-core/bin/` 性质);开发者 clone 后必须 `just plugin-pack` 才能 build,与 `just sync` 一起完成 vendor 树重建。
...
- **Build step order**: Numbered scripts in `daedalus-core/files/scripts/` run in `sort --sort=human-numeric` order (`10-base` → … → `60-ai-middleware` → `65-ai-safety` → `70-daedalus-mcp-servers` → `75-daedalus-copilot` → `76-daedalus-plugin-gen` → `91-image-info` → `cleanup.sh`).
- **Go 唯一实现 (取代旧 Python+Deno parity 条款)**: fs/shell/pkg/sysinfo 与审计仅有 Go 实现 (能力服务器在 `daedalus-plugins/`, 审计库在 `daedalus-sdk/audit/`), 不得恢复 Python/Deno 服务器。Copilot 的 `policy.ts` 内联 15 命令/9 前缀/5 blocked **冻结副本**, 与 `daedalus-sdk/shellpolicy` 存在**双向同步义务** (改一侧必改另一侧); 该契约由 `tests/deno/shellpolicy_contract.test.ts` (ALLOW_COMMANDS REPLACE 语义 Go↔Deno 一致) 与 Go 侧钉子测试共同钉住。policy.toml ↔ `policy.Default()` ↔ shellpolicy/pathguard 常量的三点防漂移链见 `daedalus-sdk/policy` 测试。
- **测试布局**: Deno 测试住仓库根 `tests/deno/` (镜像外), 相对导入指向 `daedalus-core/plugin/copilot/` 源码; Go 测试随包住 `daedalus-core/`。镜像 rootfs 内零测试文件。
...
- **本仓库边界**:本仓库装的是 daedalus 运行时(`daedalus-core/` Go 静态二进制) + 官方自带插件(`daedalus-core/plugin/` 的 copilot + `daedalus-plugins/` 的 6 能力) + 这些官方插件的运维工具(`scripts/plugin-i18n-sync.sh` 等)。**插件开发脚手架**(生成新插件骨架、`daedalus-plugin-scaffold new` 之类)属另一个仓库,本仓库不实现;**外部作者的插件**各自维护在各自仓库,通过 `daedalus-host` 加载(后续 plan)。
...
- **NEVER** leak source into the image rootfs: `daedalus-core/` (Go 源码 + 构建期下载到 GOMODCACHE 的模块) and `daedalus-core/plugin/` sources and any `*.test.ts`/`*.py`/`__pycache__`/`vendor` must never be rsync/COPY'd into `/opt` — only build products (`plugins/` 安装态, `usr/local/bin` binaries, `shared/policy.toml`) land there (`just verify-image` asserts this).
- **NEVER** hand-edit `daedalus-core/files/system/opt/daedalus/plugins/` (构建产物) — regenerate via `just plugin-pack` / `./scripts/pack-copilot-plugin.sh`.
- **NEVER** modify/commit changes to `base_image/` directly; modify `daedalus-core/` (及 `daedalus-sdk/` `daedalus-plugins/`) and run `just sync` / `./scripts/sync-daedalus.sh`.
...
- **禁止把镜像内安装态 `daedalus-core/files/system/opt/daedalus/plugins/` 手改**:必须 `just plugin-pack` / `./scripts/pack-copilot-plugin.sh` 重新生成。

## COMMANDS
```bash
# Justfile orchestration (Recommended)
just sync                  # Sync daedalus-core/{files,plugin} into base_image/ vendor tree
...
just plugin-pack           # 构建 Go 二进制 → 打包 6 能力插件 → 安装态落 files/system/opt/daedalus/plugins/ + host/audit/shell 进 usr/local/bin
...
# Deno 测试(源码在 daedalus-core/plugin/copilot/,测试在仓库根 tests/deno/;相对导入跨目录指向源码)
...
# 审计链验证 (金样向量重放)
cd daedalus-core && go run ./cmd/daedalus-audit verify --log-path ../daedalus-sdk/audit/testdata/golden.jsonl
...
## NOTES
- `daedalus-core/` (代码逻辑层 + copilot 插件 + 镜像构建层) 与 `daedalus-sdk/` `daedalus-plugins/` (独立仓根) are the authoritative source directories tracked by git. `base_image/` is gitignored vendor.
...
- **构建机限制 (todo 5/10/13 记录)**: `just build` 在本机不可执行——`almalinux-bootc:10` 要求 x86-64-v3 而本机 CPU 不支持; 镜像内断言 (`just verify-image`, in-image `daedalus-host list`, bootc lint, 76 脚本真实执行, copilot wrapper 全链路) 在 v3 构建机/CI 上补跑 (plan todo 18)。本机等价断言: `find daedalus-core/files/system base_image/files/system \( -name "*.py" -o -name "*.test.ts" -o -name "__pycache__" -o -name "go.mod" \) | wc -l` == 0。
...
- 仓库侧（源码树，0 安装）：源码侧 `daedalus-core/plugin/<短名>/`（`fs`、`shell`、`pkg`、`sysinfo`、`copilot`）的目录名与 manifest id（`daedalus.fs` 等）不一致，且源码 manifest 不含 checksums。因此直接 `-dir daedalus/plugin` 时 `list` 能运行但全部标 degraded，`inspect`/`verify`/`run-plugin`/`render-unit` 会被 id 一致性防线拒绝。完整可用的源码树形态是先打包再解包（打包会注入 checksums），从仓库根执行：
...
prefix = "./daedalus-core/files/system"
...
#     '--allow-read=./daedalus-core/files/system/opt/daedalus,/home,/var/log,...'
#     '--allow-write=/var/log/daedalus,/tmp,$HOME/.local/share/daedalus'
#     --allow-run=./daedalus-core/files/system/usr/local/bin/deno,
#                  ./daedalus-core/files/system/usr/local/bin/daedalus-audit,
...
#           daedalus-core/files/system/usr/local/bin/(deno --allow-run 旗标放行的路径)
