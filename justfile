...
plugin-pack: blueprint-embed
    ...
    for cap in fs shell pkg sysinfo service blueprint dupe; do
        ...
        if [ "${cap}" = "dupe" ]; then
            # dupe 仅 TMPDIR 解包校验、不进 files/system,理由(zip 照常产出到 core bin/):
            #   1) dupe 的安装态从未入库(git ls-files 中 daedalus.dupe/ 零条目),往入库树
            #      写 dupe 是净新增未跟踪残留;
            #      2) CI 镜像供料腿 fetch-plugins.sh 只解 6 个 zip、不含 dupe;
