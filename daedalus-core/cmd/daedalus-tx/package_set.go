...
// package_set.go —— package.set 事务适配器的类型层、边界解析、Propose 读快照与
// Apply 执行通道(plan daedalus-pkg-kind todo 6/8/9)。
//
// 本文件落"形状"与生命周期: args/before/after 快照类型 + packageSetAdapter +
// packageSetVerbs 冻结映射 + desiredStateOK / parsePackageSetArgs /
// sanitizePackageName + Propose + Apply + Rollback(todo 9/10 落地; orchestrator
// D-1 裁决: txID 经 ctx 注入, withTxID/txIDFromCtx 缝在本文件)。
// dnf/rpm 进程助手归 todo 7(package_set_exec.go), 注册进 registry 归 todo 11。
//
// desired_state 语义面(Q3 拍板 v1 边界): present|absent|latest 三值, 映射 dnf
// 动词 install|remove|upgrade(见 packageSetVerbs); 表外值由 desiredStateOK 在
// todo 8/9 拒, 错串 `unknown desired_state %q (v1 支持 present|absent|latest)`。
//
// 【oracle review C2 fix】dnf history id **不**进 packageSetBefore —— adapter
// 无法回写已持久化的 step.BeforeState; 跨步骤传递走 sidecar 文件
// (todo 7 writeDnfHistorySidecar 写, todo 9 Apply 捕获, todo 10 Rollback 读)。
//
// 【oracle review C3 fix】mock 注入契约(本 todo 预告, todo 7-10 落地): 生产代码
// 的调用点必须用五个包级 var 名 —— dnfExecFn / readDnfHistoryIDFn /
// writeDnfHistorySidecarFn / rpmQueryFn / geteuid, 绝不直接调原始函数; 测试在
// _test.go 里覆写这些 var + defer/t.Cleanup 还原。契约的承重前提是**调用点在生产
// 代码里从写下第一行起就用 var 名**, 等测试时再改 mock 就不生效了。
...
// packageSetVerbs 显式定义在 todo 6(plan 条款: 被 todo 8 Propose / todo 12
// parseArgs 测试 / todo 15 Rollback 共用): desired_state → dnf 子命令的冻结映射;
// missing key 由 desiredStateOK 在 todo 8/9 拒。
...
// 以下五个函数 var 是 mock 注入缝(oracle C3 fix), 实现函数已由 todo 7 落在
// package_set_exec.go, 本处完成移交义务的字面接线 —— 生产调用点(todo 8/9/10)
// 一律经 var 名调用, 测试在 _test.go 里覆写 + defer/t.Cleanup 还原。
// dnfHistoryExistsFn 是接线时新增的第六道缝(todo 10 Rollback 探测的 mock 契约)。
var (
	dnfExecFn                = dnfExec
...
// geteuid 是 os.Geteuid 的包级 var 注入缝(oracle C3 fix: 函数本身不可 mock,
// 必须包级 var)。todo 9/10 的 root 守门一律经本 var 调用, 绝不直用 os.Geteuid。
var geteuid = os.Geteuid

// ──── txID 的 ctx 注入缝(orchestrator D-1 裁决, todo 9 BLOCKED 取证的解法) ────
//
...
// 两处编排(D-1 裁决第 1 条豁免令划定的全部改动面); 空串原样返回(与 withUnitDir
// 同款防御——调用方语义上永不传空, 传了即视作未透传, 读侧 fail-closed)。
...
// fail-closed 守门(dnf 副作用之前)与 todo 10 Rollback 的 sidecar 寻址共用本缝;
...
// packageSetArgs 是步骤 args 的线上形态(plan 钉死的两键 schema, 未知键拒绝)。
...
// 空串 + omitempty 整键缺席。【oracle C2 fix】已废的 dnf history id 字段绝不
// 在这里 —— adapter 不能回写 step.BeforeState, dnf history id 跨步骤传递走
// sidecar 文件(todo 7/9/10)。
...
// (install|remove|upgrade); 表外 desired_state 由 todo 8 的守卫先拒, omitempty
// 只是防御性兜底。
...
// packageSetAdapter 无状态空壳: 生命周期三段已全部落地(Propose=8, Apply=9,
// Rollback=10); todo 11 才 RegisterAdapter 进 registry。
...
// (2)【oracle review I2 fix】post-checks 必需 —— 白名单字符类允许 `.`,
// 光靠 regex 挡不住 "a..b" 路径回溯与 ".hidden" 隐藏文件语义; `/` 与 `\`
// 虽已在字符类外, ContainsAny 仍显式钉住路径分隔符契约(防未来字符类放宽漏口);
// (3)【review C-1 fix】set 域在白名单之外拒通配星号(理由详见函数体内注释)。
...
func sanitizePackageName(name string) error {
	...
	// 【review C-1 fix】查询域字符类含通配星号是 dnf_query 的合法 pattern 面；
...
// Propose (todo 8, 镜像 serviceSetAdapter.Propose): 解析 → 名称门 → desired_state
...
// 不做 root 守门(那是 todo 9 Apply 的职责); 任何拒绝都发生在 rpm 查询之前,
...
func (packageSetAdapter) Propose(ctx context.Context, args json.RawMessage) (json.RawMessage, json.RawMessage, error) {
	...
	// 现状查询走 rpmQueryFn var —— oracle C3 mock 注入契约: 生产调用点自写下
	// 第一行起就用 var 名, 测试在 _test.go 里覆写 + t.Cleanup 还原。
	installed, version, err := rpmQueryFn(ctx, in.Name)
	if err != nil {
		return nil, nil, err
	}
	// before: 未装时 version 为空串, 由 omitempty 消化整键缺席(【oracle C2 fix】
	// 已废的 dnf history id 字段绝不进这里, 跨步骤传递走 sidecar 文件)。
...
// Apply (todo 9, 镜像 serviceSetAdapter.Apply 结构 + review critical #1 三连
// fail-closed): 三连重校验(args/名称/desired_state, 手改日志纵深防御, 每次独立
// 跑完)→ root 守门 → txID 透传守门(裁决第 3 条: 位于 euid 之后、dnf 直发之前,
// 拿不到 txID 时绝不先跑 dnf)→ 动词映射 → dnf 直发 → 【critical #1】history
// 捕获与 sidecar 落盘任一失败都整体判 Apply 失败——杜绝"Apply 成功但 Rollback
// 不可用"。两个 "rollback unavailable" 错误前缀逐字锁定(todo 14 守门消费)。
...
func (packageSetAdapter) Apply(ctx context.Context, step tx.Step) tx.OpResult {
	...
	// root 守门(Q1 拍板): dnf install/remove/upgrade 是系统域写操作, v1 只允许
	// root 经 sudo 走本通道; 经 geteuid var 判定(oracle C3 注入契约, 测试翻非零)。
	if geteuid() != 0 {
		return tx.OpResult{Returncode: 1, Error: "package.set requires root (euid=0); re-run via sudo"}
	}
	// txID 透传守门(D-1 裁决): sidecar 以 txID+step.Index 命名, 缺失即回滚材料
	...
	// 【review M-1 fix】透传本次实发的 verb 与包名: history 捕获经末行
	...
	// sidecar 落盘是 Apply 成功的必要条件(review critical #1 的实现基础):
	// 写失败绝不吞错续行返成功——todo 10 Rollback 全靠这份文件寻址回滚。
...
// Rollback (todo 10, 镜像 serviceSetAdapter.Rollback 的三段结构): 三连重校验 →
// root 守门 → txID 透传守门(与 Apply 逐字同串, D-1 通道复用)→ sidecar 读取门 →
// dnf history 探测分流。【review important #3】sidecar 缺失/读失败 → 严格
// fail-closed, 不探测、不兜底、绝不猜 history id;【oracle critical #1】history
// 已被清理时按 desired_state 分流 best-effort: present/latest → remove 兜底;
// absent → **严格 error 且 dnf 物理零调用**——dnf remove 对已 absent 包是
// 空操作会误报回滚成功, dnf install 又需要版本信息, 强制人工处理。
// 纯消费侧契约: 不写 sidecar、不回写 BeforeState、不重建 history; undo/兜底的
// dnf 结果原样透传(rc!=0 即失败, 不吞)。sidecar 读取门必须排在探测之前
// (顺序即契约, todo 15 E2E 消费)。诊断细节走 OpResult.Stderr, Error 保持逐字。
func (packageSetAdapter) Rollback(ctx context.Context, step tx.Step) tx.OpResult {
	...
	// sidecar 读取门(review important #3): 该文件是 Apply 成功的必要条件产物,
	...
	if err != nil {
		// 探测不可靠(plan 未规定三态): 存在性未知即不动 dnf, fail-closed 上抛。
	...
	default:
		...
		// 不可逆严格 error, dnf 物理零调用(oracle critical #1 守门)。
